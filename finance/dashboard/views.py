...
def transaction_dynamics(request):
    ...
    transactions = Transaction.objects.filter(
        ...
        'date': "date(date_time)",
        'week': "strftime('%Y-%W', date_time)",
        'month': "strftime('%Y-%m', date_time)"
...
def bank_stats(request):
    ...
    sender_stats = Transaction.objects.filter(
        user=request.user
    ).values('sender_bank__name').annotate(
        count=Count('id'),
        total_amount=Sum('amount')
    )

    receiver_stats = Transaction.objects.filter(
        user=request.user
    ).values('receiver_bank__name').annotate(
        count=Count('id'),
        total_amount=Sum('amount')
    )
